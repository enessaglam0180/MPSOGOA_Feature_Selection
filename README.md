...
Social interactions among grasshoppers (attraction/repulsion forces).

Their personal historical best position ($p_{\text{best}}$).

The global swarm's best position ($g_{\text{best}}$).
...
$$T(x) = \left| \text{erf}\left(\frac{\sqrt{\pi}}{2} x\right) \right| \quad \text{or} \quad \left| \frac{x}{\sqrt{1+x^2}} \right|$$
...
Swarm / Population size (Number of search agents)
...
Max parameter controlling GOA's comfort zone decay

$1.0$

cMin

Min parameter controlling GOA's comfort zone decay

$0.00001$

w_inertia

PSO inertia weight factor
...
Console Summary: Accurate details on the baseline accuracy (using all features) compared to the post-optimization performance, highlighting the reduction in feature dimensionality.

Convergence Charts (comparison.png): Shows how quickly bMPSOGOA converges to high-accuracy states compared to pure PSO and GOA.

Transfer Function Performance Plots: Visual verification of S-shaped vs. V-shaped binarization performance in terms of exploration stability and feature preservation.
